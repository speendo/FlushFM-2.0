...
- **Interaction:** Communicate exclusively through public interfaces; use FreeRTOS queues for cross-core messages, callbacks for same-core communication; in a Supervisor Pattern, components push state actively via callback rather than being observed passively (→ `concurrency.md`, `state-management.md`); never reach into another component's internals
